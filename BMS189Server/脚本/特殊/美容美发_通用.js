    ...
    if (mode == -1) {
        ...
        if (status <= i++) {
            ...
            if (status === i++) {
                ...
                text += 心形图标 + '#b#L0#发型设计#l	';
                text += 心形图标 + '#b#L2#发型染色#l	';
                text += 心形图标 + '#b#L6#混合染色#l	';
                text += 心形图标 + '#b#L7#随机混染#l	\r\n\r\n';
                text += 心形图标 + '#b#L1#脸部护理#l	';
                text += 心形图标 + '#b#L3#皮肤染色#l	';
                text += 心形图标 + '#b#L4#眼睛染色#l	';
                text += 心形图标 + '#b#L5#机器美容#l	\r\n';
...
function action0(f, j, l, e) {
    var k = cm.getConstantScriptContent('菜单_NPC立绘');
    var g = Math.floor(Math.random() * k.length);
    var b = k[g][0];
    var h = randomNum(0, k[g][1]);
    if (status <= e++) {
        var m = '#face' + h + '##fs18##r#fn方正粗黑宋简体#';
        m += '#b请选择理发分类：#k\r\n';
        m += '#fs14#(点券收费：随机' + 随机理发价格 + ' 自选' + 自选理发价格 + ' 定制' + 自选理发价格 * 2 + ')\r\n\r\n#fs18#';
        m += ' #L0##r' + one + ' 随机发型 #l	#L1##r' + two + ' 自选发型 #l	#L2##r' + three + ' 代码定制 #l\r\n\r\n\r\n';
        m += '#L10086#返回上一页#l';
        ...
        if (status === e++) {
            if (cm.getzhizunvip() == 2) {
                ...
                var m = '亲爱的#b#h ##k，欢迎来到#rBMS至尊版美发中心#k。\r\n';
            } else {
                if (cm.getPlayer().getGender() == 0) {
                    发型列表 = 黄金男发型;
                } else {
                ...
                var m = '亲爱的#b#h ##k，欢迎来到#rBMS黄金版美发中心#k。\r\n（至尊版解锁全部造型）\r\n';
            }
            if (l == 0) {
                美发类型 = 1;
                var m = '#face' + h + '##fs18##r#fn方正粗黑宋简体#';
                m += '好的，那么你想看看本期发型沙龙系列吗？\r\n\r\n#r#L1#更换发型#l\r\n#L0#好把, 看看这都有什么发型#l';
                cm.askMenu_Bottom(m, 204, b);
            } else {
                if (l == 1) {
                    var a = cm.getJob();
...
function action1(g, k, m, e) {
    var l = cm.getConstantScriptContent('菜单_NPC立绘');
    var h = Math.floor(Math.random() * l.length);
    var b = l[h][0];
    var j = randomNum(0, l[h][1]);
    if (status <= e++) {
        var n = '#face' + j + '##fs18##r#fn方正粗黑宋简体#';
        n += '#b请选择美容分类：#k\r\n';
        n += '#fs14#(点券收费：随机' + 随机美容价格 + ' 自选' + 自选美容价格 + ')\r\n\r\n#fs18#';
        n += ' #L0##r' + one + ' 随机美容 #l	#L1##r' + two + ' 自选美容 #l	#L2##r' + two + ' 代码定制 #l\r\n\r\n\r\n';
        n += '#L10086#返回上一页#l';
        cm.askMenu_Bottom(n, 204, b);
    } else {
        if (status === e++) {
            if (cm.getzhizunvip() == 2) {
                if (cm.getPlayer().getGender() == 0) {
            ...
            if (m == 0) {
                ...
                if (m == 1) {
                    ...
                    if (m == 2) {
                        美容类型 = 3;
                        cm.askText_Bottom('#face' + j + '##fn方正粗黑宋简体# 请输入您要切换的脸型ID或名称，支持模糊查询。#fs15#\r\n  #b如：双瞳色、未来战士、恶魔之、漫画', b);
                    } else {
                        if (m == 10086) {
                            cm.dispose();
                            cm.openNpc(0, '美容美发_通用');
...
function action6(d, c, b, a) {
    if (status <= a++) {
        if (cm.getPlayer().getCSPoints(1) >= 随机理发价格) {
            cm.getPlayer().modifyCSPoints(1, -随机理发价格, true);
            cm.头发混染('可以混合两种颜色来改变发色，选择基础色和混合色，移动滚轮打造专属于自己的有颜色吧。');
        } else {
            cm.playerMessage(-1, '好像没有钱？等有钱了再来吧！');
            cm.dispose();
        }
        ...
        if (status === a++) {
            cm.effect_Text(['#fn黑体 ExtraBold##fs26# 混合美发成功'], [
                85,
                2500,
                4,
                0,
            ...
            cm.forceStartQuest(10000086);
            cm.forceCompleteQuest(10000086);
            cm.sendOk('混染完成了！快来欣赏一下你的新发型吧！', 1012103);
            cm.dispose();
        } else {
            cm.dispose();
        }
...
function action7(d, c, b, a) {
    if (status <= a++) {
        if (cm.getPlayer().getCSPoints(1) >= 随机理发价格) {
            ...
            cm.forceStartQuest(10000086);
            cm.forceCompleteQuest(10000086);
            cm.sendOk('混染完成了！快来欣赏一下你的美丽发型吧！', 1012103);
            cm.dispose();
        } else {
            cm.playerMessage(-1, '好像没有钱？等有钱了再来吧！');
            cm.dispose();
