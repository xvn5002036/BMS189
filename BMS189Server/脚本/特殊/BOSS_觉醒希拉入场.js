...
function action(h, j, l) {
    ...
    if (cm.getMapId() == startmap) {
        if (status == 0) {
            ...
            cm.askYesNo(o + '           #b#h0# #k#e是否现在就进入？#n', 3003810);
        } else {
            if (status == 1) {
                if (cm.getLimitBreak() < minLimitBreak[chs]) {
                    cm.sendOk('你的武器破功' + cm.getLimitBreak() + '不够' + minLimitBreak[chs] + '，不允许进入！', 3003810);
                    cm.dispose();
                } else {
                    if (!cm.isLeader()) {
                        cm.sendOk('嗯，你已经有自己的队伍。要执行这个任务，请让你们的队长与我对话，才可以申请入场.', 3003810);
                        cm.dispose();
                        return;
                    } else {
                        if (!cm.isAllPartyMembersAllowedLevel(minLevel[chs], maxLevel[chs])) {
                            cm.sendNext('组队成员等级 ' + minLevel[chs] + ' 以上 ' + maxLevel[chs] + ' 以下才可以入场。', 3003810);
                            cm.dispose();
                        } else {
                            if (channel[chs] != 0 && cm.getPlayer().getClient().getChannel() != channel[chs]) {
                                cm.sendNext('此副本需要在 ' + channel[chs] + '频道进入.', 3003810);
                                cm.dispose();
                            } else {
                                if (!cm.isAllPartyMembersAllowedPQ(PQLog[chs], maxenter[chs])) {
                                    cm.sendNext('你的队员#r#e "' + cm.getNotAllowedPQMemberName(PQLog[chs], maxenter[chs]) + '" #k#n次数已经达到上限了。', 3003810);
                                    cm.dispose();
                                } else {
                                    if (!cm.allMembersHere()) {
                                        cm.sendOk('你的组队部分成员不在当前地图,请召集他们过来后在尝试。', 3003810);
                                        cm.dispose();
                                    } else {
                                        ...
                                        if (g) {
                                            ...
                                            if (b == null || open == false) {
                                                cm.askMenu(open ? '配置文件不存在,请联系管理员。' : '管理员关闭了目前的副本.有问题可以联系管理员', 3003810);
                                            } else {
                                                var a = b.getNumberProperty('state');
                                                if (a == 0) {
                                                    cm.dispose();
                                                    ...
                                                    cm.sendOk('已经有队伍在进行了,请换其他频道尝试。', 3003810);
                                                    cm.dispose();
                                                }
                                                cm.dispose();
                                            }
                                        } else {
                                            cm.sendOk('组队成员 ' + minPlayers[chs] + ' 人以上 ' + maxPlayers[chs] + '人 以下才可以入场。', 3003810);
                                            cm.dispose();
                                        }
                                    }
                                }
