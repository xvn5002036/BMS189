...
function action(e, f, h) {
    ...
    var d = 男美容列表[Math.floor(Math.random() * 男美容列表.length)];
    var b = 女美容列表[Math.floor(Math.random() * 女美容列表.length)];
    var a = cm.getPlayer().getFace();
    var j = cm.getPlayer().getSecondFace();
    var g = 2;
    if (status == 0 && e == 0) {
        cm.dispose();
        return;
    ...
    if (status <= c++) {
        ...
        if (status === c++) {
            if (cm.getPlayer().getCSPoints(1) >= 随机美容价格) {
                ...
                cm.getTopMsgFont('随机美容成功', 3, 20, 20, 0);
                cm.addTrait('魅力', 50);
                cm.forceStartQuest(10000086);
                cm.forceCompleteQuest(10000086);
                cm.set神之子通用脸型(d, b);
                cm.神之子更换完发型脸型对比UI(造型卡, g, j, b, a, d);
                cm.removeItem(造型卡);
                cm.dispose();
            } else {
