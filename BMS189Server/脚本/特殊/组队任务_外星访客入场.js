...
function action开始地图(f, h, j) {
    ...
    if (status <= e++) {
        ...
        if (status === e++) {
            var m = '#e<' + BossName + '>#n\r\n\r\n';
            m += '这宇宙飞船内部存在奇怪的磁场！参与实验的冒险家太少了…\r\n#b';
            m += '#b#L11#询问事件#l#k\r\n';
            m += '#e#b#L0#进入宇宙飞船内部。#l#n#K\r\n';
            cm.askMenu(m);
        } else {
            if (status === e++) {
                switch (j) {
                ...
                case 11:
                    cm.dispose();
                    cm.openNpc(9390008, '组队任务_外向访客_询问事件');
                    break;
                case 12:
