...
function action开始地图(e, g, h) {
    ...
    if (status <= d++) {
        ...
        if (status === d++) {
            var l = '#e<' + BossName + '>#n\r\n\r\n';
            l += '一股神秘的力量毒害了森林，阿尔泰营地的所有士兵都忙得无暇顾及。我们需要你！\r\n#b';
            l += '#L0#进入毒雾森林。#l\r\n';
            l += '#L10#寻找同行的组队。#l\r\n';
            l += '#L11#请详细地说说是怎么回事吧。#l\r\n';
            l += '#L13#我想要#t1032101#。#l\r\n';
            ...
            if (status === d++) {
                ...
                case 10:
                    ...
                    break;
                case 11:
                    cm.sendNormalTalk('这曾经是一片干净、宁静的妖精森林。但是不久前，一个穿着黑色长袍的神秘人来了，赶走了妖精们，开始了一项奇怪的研究。由于他的研究，森林的污染越来越严重。我们现在必须拯救森林！', 0, 2133000, false, true);
                    status = -1;
                    break;
                case 12:
                    ...
                    break;
                case 13:
                    cm.sendNext('正好我这有多余的，送你一副吧。');
                    cm.playerMessage(-1, '获得了1个闪耀阿尔泰耳环道具。（装备）');
                                            ...
                                            if (cm.getPartyMembers().size() < 最小人数[chs] || cm.getPartyMembers().size() > 最大人数[chs]) {
                                                ...
                                                if (c == 0) {
                                                    ...
                                                    f.getPlayers().forEach(function (i) {
                                                        ...
                                                        i.getAPI().removeAll(4001161);
                                                        i.getAPI().removeAll(4001162);
                                                        i.getAPI().removeAll(2270004);
                                                        i.getAPI().removeAll(4001169);
                                                        i.getAPI().removeAll(4001163);
                                                        i.getAPI().removeAll(4001164);
                                                    });
...
function action其他地图(d, c, b) {
    ...
    if (status <= a++) {
        ...
        if (status === a++) {
            cm.sendNormalTalk('请和我一起去很深的妖精森林吧。', 0, 2133000, false, true);
        } else {
            if (status === a++) {
